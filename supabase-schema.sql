...
-- Email domain validation constraint (removed for development)
...
DROP POLICY IF EXISTS "Instructors can read messages" ON public.gc_messages;
DROP POLICY IF EXISTS "Instructors can send messages" ON public.gc_messages;
...
-- Function to validate email and determine role (domain restriction removed for development)
...
BEGIN
    ...
    -- Domain validation removed for development/testing
    -- IF v_domain != 'nbsc.edu.ph' THEN
    --     RAISE EXCEPTION 'Email domain must be @nbsc.edu.ph';
    -- END IF;

    -- For NBSC emails, determine role by whether local part is numeric
    -- For non-NBSC emails, default to instructor for development
    IF v_domain = 'nbsc.edu.ph' THEN
        ...
        END IF;
    ELSE
        -- Non-NBSC emails default to instructor for development
        v_role := 'instructor';
    END IF;
...
-- ============================================
-- TEST DATA FOR INSTRUCTOR UI
-- ============================================

-- NOTE: Before running this test data, you need to create Supabase Auth users
-- The profiles will be automatically created by the handle_new_user trigger
-- After creating the auth users, the script will automatically find their profiles

-- Get instructor profile IDs and create test data
DO $$
DECLARE
  v_instructor_id_1 UUID;
  v_instructor_id_2 UUID;
BEGIN
  -- Try to get the first instructor profile ID (20231025@nbsc.edu.ph)
  SELECT id INTO v_instructor_id_1 FROM public.profiles WHERE email = '20231025@nbsc.edu.ph' AND role = 'instructor';
  
  -- Try to get the second instructor profile ID (nekochii57@gmail.com)
  SELECT id INTO v_instructor_id_2 FROM public.profiles WHERE email = 'nekochii57@gmail.com' AND role = 'instructor';
  
  -- Create test data for first instructor if profile exists
  IF v_instructor_id_1 IS NOT NULL THEN
    -- Insert test courses for the instructor
    INSERT INTO public.courses (code, title, description, instructor_id, department, credits, max_students, current_students, semester, academic_year, is_active)
    VALUES
      ('IT311', 'System Integration and Architecture', 'Advanced system design and integration patterns', v_instructor_id_1, 'Institute for Computer Studies (ICS)', 3, 40, 32, '1st Semester', '2024-2025', true),
      ('IT312', 'Information Assurance and Security', 'Network security and information protection', v_instructor_id_1, 'Institute for Computer Studies (ICS)', 3, 40, 0, '1st Semester', '2024-2025', true),
      ('IT301', 'Application Development and Emerging Technologies', 'Modern app development with cutting-edge technologies', v_instructor_id_1, 'Institute for Computer Studies (ICS)', 3, 40, 28, '1st Semester', '2024-2025', true)
    ON CONFLICT (code) DO NOTHING;
    
    -- Insert test sections for the courses
    INSERT INTO public.sections (course_id, name, instructor_id, room, max_capacity, current_enrollment)
    SELECT 
      c.id,
      CASE c.code
        WHEN 'IT311' THEN 'BSIT 3A'
        WHEN 'IT312' THEN 'BSIT 3B'
        WHEN 'IT301' THEN 'BSIT 3A'
      END,
      v_instructor_id_1,
      'ICT-2',
      40,
      ...
      END
    FROM public.courses c
    WHERE c.instructor_id = v_instructor_id_1
    ON CONFLICT (course_id, name) DO NOTHING;
    
    -- Insert test messages for IT311 course
    INSERT INTO public.gc_messages (course_id, sender_id, content, created_at)
    SELECT 
      c.id,
      v_instructor_id_1,
      'Welcome to IT311! Post your questions here anytime.',
      NOW() - INTERVAL '3 hours'
    FROM public.courses c
    WHERE c.code = 'IT311' AND c.instructor_id = v_instructor_id_1
    ...
    SELECT 
      c.id,
      v_instructor_id_1,
      '9AM as usual, room ICT-2.',
      NOW() - INTERVAL '40 minutes'
    FROM public.courses c
    WHERE c.code = 'IT311' AND c.instructor_id = v_instructor_id_1
    ON CONFLICT DO NOTHING;
    
    -- Insert test message for IT301 course
    INSERT INTO public.gc_messages (course_id, sender_id, content, created_at)
    SELECT 
      c.id,
      v_instructor_id_1,
      'Reminder: bring your laptops next meeting.',
      NOW() - INTERVAL '10 hours'
    FROM public.courses c
    WHERE c.code = 'IT301' AND c.instructor_id = v_instructor_id_1
    ON CONFLICT DO NOTHING;
    
    RAISE NOTICE 'Test data inserted successfully for instructor 20231025@nbsc.edu.ph';
  ELSE
    RAISE NOTICE 'Instructor profile 20231025@nbsc.edu.ph not found. Please create auth user first.';
  END IF;
  
  -- Create test data for second instructor if profile exists
  IF v_instructor_id_2 IS NOT NULL THEN
    -- Insert test courses for the second instructor
    INSERT INTO public.courses (code, title, description, instructor_id, department, credits, max_students, current_students, semester, academic_year, is_active)
    VALUES
      ('CS101', 'Introduction to Computer Science', 'Fundamentals of computer science and programming', v_instructor_id_2, 'Institute for Computer Studies (ICS)', 3, 40, 35, '1st Semester', '2024-2025', true),
      ('CS201', 'Data Structures and Algorithms', 'Advanced data structures and algorithm analysis', v_instructor_id_2, 'Institute for Computer Studies (ICS)', 3, 40, 30, '1st Semester', '2024-2025', true)
    ON CONFLICT (code) DO NOTHING;
    ...
    SELECT 
      c.id,
      CASE c.code
        WHEN 'CS101' THEN 'BSCS 1A'
        WHEN 'CS201' THEN 'BSCS 2A'
      END,
      v_instructor_id_2,
      'ICT-3',
      40,
      ...
      END
    FROM public.courses c
    WHERE c.instructor_id = v_instructor_id_2
    ON CONFLICT (course_id, name) DO NOTHING;
    
    -- Insert test messages for CS101 course
    INSERT INTO public.gc_messages (course_id, sender_id, content, created_at)
    SELECT 
      c.id,
      v_instructor_id_2,
      'Welcome to CS101! Let us learn programming together.',
      NOW() - INTERVAL '2 hours'
    FROM public.courses c
    WHERE c.code = 'CS101' AND c.instructor_id = v_instructor_id_2
    ON CONFLICT DO NOTHING;
    
    -- Insert test message for CS201 course
    INSERT INTO public.gc_messages (course_id, sender_id, content, created_at)
    SELECT 
      c.id,
      v_instructor_id_2,
      'Please review the sorting algorithms before our next class.',
      NOW() - INTERVAL '1 hour'
    FROM public.courses c
    WHERE c.code = 'CS201' AND c.instructor_id = v_instructor_id_2
    ON CONFLICT DO NOTHING;
    
    RAISE NOTICE 'Test data inserted successfully for instructor nekochii57@gmail.com';
  ELSE
    RAISE NOTICE 'Instructor profile nekochii57@gmail.com not found. Please create auth user first.';
  END IF;
END $$;
