...
CREATE POLICY "section_enrollments_select_instructor" ON public.section_enrollments
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.sections s
            WHERE s.id = public.section_enrollments.section_id AND s.instructor_id = auth.uid()
        )
    );
...
BEGIN
    ...
    INSERT INTO public.profiles (id, email, role, verified, student_id)
    VALUES (
        NEW.id,
        NEW.email,
        guessed_role,
